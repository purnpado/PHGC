...
if (Test-Path $versionFile) {
    ...
    $json.downloadUrl = "https://github.com/purnpado/PHGC/releases/latest/download/PHGC.exe"
...
# Git 푸시
Write-Host ">>> Git 저장소($gitRemote) 푸시 중..." -ForegroundColor Cyan
git push $gitRemote main -f
git push $gitRemote "v$newVer" -f
...
# ===== 5. Gitea Release 생성 및 exe 업로드 (설정된 경우에만 실행) =====
$giteaURL = $env:GITEA_URL
$giteaOwner = $env:GITEA_OWNER
$giteaRepo = $env:GITEA_REPO
if (-not $giteaRepo) { $giteaRepo = "PHGC" }

function Upload-GiteaRelease($repo) {
    if (-not $giteaURL -or -not $giteaToken -or -not $giteaOwner) {
        return
    }
    Write-Host ">>> Git 릴리즈 생성 및 PHGC.exe 업로드 중 ($repo)..." -ForegroundColor Cyan
    $headers = @{
    ...
    try {
        ...
        Write-Host ">>> Git [$repo] PHGC.exe 릴리즈 업로드 완료!" -ForegroundColor Green
    } catch {
        Write-Host ">>> Git [$repo] 릴리즈 업로드 예외: $_" -ForegroundColor Yellow
    }
}

if ($giteaURL -and $giteaToken -and $giteaOwner) {
